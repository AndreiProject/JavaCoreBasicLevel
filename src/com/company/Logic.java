...
public class Logic {
    ...
    public static void go(Result callback) {
        gameFinished = true;
        ...
        if (checkWinLines(DOT_X)) {
            ...
            callback.endGame("Ты победил!");
            return;
        }

        if (isFull()) {
            System.out.println("Ничья");
            callback.endGame("Ничья");
            return;
        }
        ...
        if (checkWinLines(DOT_O)) {
            ...
            callback.endGame("Компьютер победил!");
            return;
        }
    ...
    public static void setHumanCoords(int x, int y, Result callback) {
        if (isCellValid(y, x)) {
            map[y][x] = DOT_X;
            go(callback);
        }
